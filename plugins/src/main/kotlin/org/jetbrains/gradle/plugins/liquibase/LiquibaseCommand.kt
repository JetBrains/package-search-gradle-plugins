...
enum class LiquibaseCommand(val command: String, val description: String, val requiresValue: Boolean = false) {
    ...
    ROLLBACK(
        "rollback",
        "Rolls back the database to the state it was in when the <liquibaseCommandValue> tag was applied.",
        true
    ),
    ROLLBACK_COUNT("rollbackCount", "Rolls back the last <liquibaseCommandValue> change sets.", true),
    ROLLBACK_COUNT_SQL(
        "rollbackCountSQL",
        "Writes SQL to roll back the last <liquibaseCommandValue> change sets to STDOUT.",
        true
    ),
    ROLLBACK_SQL(
        "rollbackSQL",
    ...
    ROLLBACK_TO_DATE(
        "rollbackToDate",
        "Rolls back the database to the state it was in at the <liquibaseCommandValue> date/time.",
        true
    ),
    ...
    UPDATE_COUNT_SQL(
        "updateCountSql",
        "Writes SQL to apply the next <liquibaseCommandValue> change sets to STDOUT.",
        true
    ),
